...
const list = [
  ...
  },
  {
    ...
    title: 'Star Anise (Illicium verum)',
...
const Profiles = () => {
  return (
    <section id="profiles" className="py-10 pb-20 bg-gray-100">
      <Container>
        <div className="">
          <article className="grid gap-2 text-center md:text-center items-center justify-center mb-10">
            <h3 className="flex flex-wrap gap-3 items-center justify-center md:justify-center text-5xl font-light text-purple-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1.5fr_1fr] gap-4">
            <figure className="grid grid-cols-2 gap-5">
              {list.map((item) => (
                <figure
                  key={item.id}
                  className="overflow-hidden w-full h-[15rem] rounded-xl"
                >
                  <Image
            ...
            <ul className="flex flex-col gap-4">
              {categories.map((cat) => (
                <li
                  className="border bg-white border-gray-200 rounded-xl p-8 text-2xl font-light"
                  key={cat.id}
                >
                  <h4>{cat.title}</h4>
